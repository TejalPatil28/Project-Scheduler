...
function setTopbar(title, showBack, backFn) {
  ...
  if (backSlot) {
    backSlot.innerHTML = showBack
      ? '<button class="back-btn" id="back-btn-el">&larr; Back</button>'
      : "";
    if (showBack && backFn) {
      document.getElementById("back-btn-el").addEventListener("click", backFn);
    }
  }
}
...
function renderProjectPage() {
  ...
  container.innerHTML = [
    '<div class="proj-body-layout">',
      '<div class="xl-info-panel">',
        '<div class="xi-grid">' + leftHTML + '</div>',
      '</div>',
      '<div class="proj-body-right">',
...
function renderExcelMirror(container, data) {
  ...
  // ── Resolve border css ──────────────────────────────────
  function borderStyle(width, color) {
    var w = width === "medium" ? "2px" : width === "thick" ? "3px" : "1px";
    var c = width === "medium" ? "#8c8c8c" : width === "thick" ? "#595959" : "#BFBFBF";
    return w + " solid " + c;
  }
  ...
  var html = '<div style="overflow:auto;max-height:70vh;position:relative">';
  html += '<table style="' + tableStyle + '">';

  // ── THEAD: sticky column letters ──
  html += '<thead><tr>';
  html += '<th style="position:sticky;top:0;left:0;z-index:5;background:#e8e8e8;border:1px solid #d0d0d0;min-width:28px;width:28px;font-size:10px;font-weight:normal"></th>';
  for (var ci = 0; ci < cols.length; ci++) {
    var col = cols[ci];
    var cw  = colWidths[col] || 64;
    html += '<th style="position:sticky;top:0;z-index:4;background:#f2f2f2;color:#444;text-align:center;font-size:10px;font-weight:normal;border:1px solid #d0d0d0;width:' + cw + 'px;min-width:' + cw + 'px">' + col + '</th>';
  }
  html += '</tr></thead>';
  ...
  for (var r = 1; r <= maxRow; r++) {
    ...
    html += '<td style="position:sticky;left:0;z-index:3;background:#f2f2f2;color:#666;text-align:center;font-size:10px;border:1px solid #d0d0d0;min-width:28px;width:28px;user-select:none">' + r + '</td>';
    ...
    for (var ci2 = 0; ci2 < cols.length; ci2++) {
      ...
      // merged slave — skip (already covered by master's rowspan/colspan)
...
async function saveChanges() {
  ...
  var updates = Object.values(state.pendingChanges);
  if (!updates.length) return;
  btn.disabled = true;
  btn.innerHTML = '<span class="spinner" style="width:13px;height:13px;border-width:2px"></span>';
  try {
    await API.saveTasks(state.project.id, updates);
    ...
    state.pendingChanges = {};
    updateSaveBar();
    toast("Saved " + updates.length + " task" + (updates.length > 1 ? "s" : "") + " \u2713");
