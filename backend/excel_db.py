...
def update_tasks_bulk(project_id, updates):
    ...
    updates = list of {task_key, percent_complete, remark}
    task_key = "task_code_taskname"
    """
    fname = project_id + ".xlsx"
    fpath = os.path.join(PROJECTS_DIR, fname)
    if not os.path.exists(fpath):
    ...
    update_map = {u["task_key"]: u for u in updates}
    updated = 0
    ...
    for row in range(TASK_START_ROW, TASK_END_ROW + 1):
        ...
        if row_key in update_map:
            u = update_map[row_key]
    ...
    # Update JSON cache in-place instead of deleting it
    # Apply the same changes to cached tasks so next open is instant
    try:
        cache_path = _tasks_cache_path(project_id)
...
def get_raw_sheet(filepath, max_col=38):
    """Read every cell up to max_col columns. Returns dict for Excel-mirror UI."""
